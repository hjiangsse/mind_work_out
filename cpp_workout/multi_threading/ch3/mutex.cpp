...
void InsertStringThreadSafeList(ThreadSafeList *lst, std::string str) {
    lst->add_to_list(str);
}
...
int main(int argc, char **argv) {
	...
	threads.push_back(std::thread(InsertStringThreadSafeList, &m_list, str));
    }

    std::for_each(threads.begin(), threads.end(), std::mem_fn(&std::thread::join));
    PrintThreadSafeList(m_list);
    return 0;
}
